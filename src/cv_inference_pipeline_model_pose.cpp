...
// Load DNN OpenPose Model
cv::dnn::Net loadOpenPose() {
    std::string modelConfig = "../weights/pose.prototxt";
    std::string modelWeights = "../weights/pose.caffemodel";
    return cv::dnn::readNetFromCaffe(modelConfig, modelWeights);
}

// Function to detect human pose using OpenPose model
void detectPose(cv::dnn::Net& net, cv::Mat& frame) {
    int inWidth = 368;
    int inHeight = 368;
    float threshold = 0.1;

    // Convert frame into a blob for DNN processing
    cv::Mat blob = cv::dnn::blobFromImage(frame, 1.0 / 255, cv::Size(inWidth, inHeight), cv::Scalar(0, 0, 0), false, false);
    net.setInput(blob);
    cv::Mat output = net.forward();

    int H = output.size[2]; // Heatmap height
    int W = output.size[3]; // Heatmap width

    // Store detected keypoints
    std::vector<cv::Point> points(18);  // OpenPose COCO Model has 18 keypoints
    ...
    for (int i = 0; i < 18; i++) {
        ...
        cv::Point maxLoc;
        double maxVal;
        cv::minMaxLoc(probMap, 0, &maxVal, 0, &maxLoc);

        int x = (frame.cols * maxLoc.x) / W;
        int y = (frame.rows * maxLoc.y) / H;

        // Store keypoint if confidence is above threshold
        if (maxVal > threshold)
            points[i] = cv::Point(x, y);
        else
            points[i] = cv::Point(-1, -1);
    }

    // Draw skeleton
    for (const auto& pair : POSE_PAIRS) {
        cv::Point partA = points[pair.first];
        cv::Point partB = points[pair.second];

        if (partA.x != -1 && partB.x != -1) {
...
void run_pipeline(const std::string& source,
    ...
    while (cap.isOpened() && (duration < 0 || duration > duration_cast<seconds>(steady_clock::now() - time_limit).count())) {
        ...
        // Inference Processing (Run pose estimation)
        auto inference_speed_start = steady_clock::now();
        detectPose(net, frame);
...
int main(int argc, char* argv[]) {
    std::string video_path = "../assets/person_1.mp4";
