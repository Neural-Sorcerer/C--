...
cv::dnn::Net loadFaceDetector() {
    std::string modelFile = "../weights/face.caffemodel";
    std::string configFile = "../weights/face.prototxt";
    return cv::dnn::readNetFromCaffe(configFile, modelFile);
}

// Face Detection Function
std::vector<cv::Rect> detectFaces(cv::dnn::Net& net, cv::Mat& frame) {
    cv::Mat blob = cv::dnn::blobFromImage(frame, 1.0, cv::Size(300, 300), cv::Scalar(104, 177, 123), false, false);
    net.setInput(blob);
    cv::Mat output = net.forward();

    std::vector<cv::Rect> faces;
    float* data = (float*)output.ptr(0);    // ✅ Get pointer to first element
...
void run_pipeline(const std::string& source,
    ...
    // Load Face Detector
    cv::dnn::Net faceDetector = loadFaceDetector();
    ...
    while (cap.isOpened() && (duration < 0 || duration > duration_cast<seconds>(steady_clock::now() - time_limit).count())) {
        ...
        // Inference Processing (Detect Faces)
        auto inference_speed_start = steady_clock::now();
        std::vector<cv::Rect> faces = detectFaces(faceDetector, frame);
